...
def dispatch(task_list, message):
    """Dispatches a server message to the appropriate task_list method."""
    ...
    if message["method"] == "query":
        ...
        if payload is None or "note" not in payload or "task_id" not in payload:
...
class Server:
    ...
    def __init__(
        ...
        if self.addr.endswith(".sock") or unix_socket is not None:
            self.coro = self.loop.create_unix_server(self.factory, path=self.addr)
        else:
